

const SkillCard = () => {
  return (
    <>
      <div className="flex flex-col max-w-64 border-4 border-[#494d56]">
        {/* Header section */}
        {/* <div
          className="grid grid-cols-2 border-b-2 p-4"
        >
          <div className="grid justify-center items-center text-lg">
            ChertNodes
          </div>
          <div className="grid grid-rows-3 justify-start">
            <div className="text-white text-sm">ABC</div>
            <div className="text-white text-sm">ABC</div>
            <div className="text-white text-sm">ABC</div>
          </div>
        </div> */}
        <div className="border-b-2 border-[#494d56] h-52">
          <img src="data:image/png;base64,"
           className=" w-full h-full object-fill"
           alt='logo'/>
        </div>
        {/* Middle Section */}
        <div className="flex flex-row items-center border-b-2 border-[#494d56] text-slate-400 p-1">
          HTML CSS Python Flask
        </div>
        {/* Footer Section */}
        <div className="p-4">
          <h1 className="text-white text-lg py-3">ChertNodes</h1>
          <p className="text-slate-400 text-sm pb-3">Server hosting</p>
          <div className="flex flex-row justify-start gap-10">
            {/* <Button></Button>
            <Button></Button> */}
            <div className="border border-[#775288] px-2 py-1 text-white text-sm w-1/4 text-center">Live</div>
            <div className="border border-[#775288] px-2 py-1 text-white text-sm w-1/4 text-center">Live</div>
          </div>
        </div>
      </div>
    </>
  );
};

export default SkillCard;
